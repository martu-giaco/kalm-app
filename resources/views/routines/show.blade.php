    ...
    <section class="pt-10 pb-20 bg-white rounded-t-3xl">
        <section class="px-5 mb-6">
            @if ($routine->is_reminder_enabled)
                ...
                @else
                    <span
                        class="inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold rounded-full text-amber-700 bg-amber-100">
                        Pendiente
                    </span>
                @endif
            @endif
            ...
            @if ($routine->is_reminder_enabled && !$routine->is_completed_today)
                        <div class="flex gap-2 px-5 mt-4">
                            <form action="{{ route('routines.complete', $routine->routine_id) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="px-4 py-2 text-sm font-bold text-white bg-green-600 rounded-xl">
                            ...
                            <form action="{{ route('routines.postpone', $routine->routine_id) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="px-4 py-2 text-sm font-bold text-white rounded-xl bg-amber-500">
                                    Posponer 15 min
                                </button>
                            </form>
                        </div>
                    ...
                    <div class="flex flex-col gap-3">
                        @foreach ($step['products'] as $product)
                            ...
                            <dialog id="desc_paso_{{ $step['number'] }}" class="modal">
                                <div class="modal-box">
                                    <h3 class="text-lg font-bold">Paso {{ $step['number'] }}: {{ $step['title'] }}
                                    </h3>
                                    <p class="py-4">{{ $step['products']->first()->category?->description }}</p>
                                </div>
    ...
    <dialog id="menu_rutina_{{ $routine->id }}" class="modal modal-bottom">
        <div class="modal-box">
            <a href="{{ route('routines.edit', $routine) }}"
                class=" btn w-full inline-flex border-0 bg-[#CCE2E5] px-6 py-3 rounded-xl font-semiboldbold transition-all duration-300 items-center justify-between gap-2 text-sm font-bold">
                <p>Editar Rutina</p>
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"
                    fill="#306067">
                    <path
            ...
            <form action="{{ route('routines.destroy', $routine) }}" method="POST"
                ...
                @method('DELETE')
                <button type="submit"
                    class="mt-3 btn w-full inline-flex border-0 bg-[#741919] px-6 py-3 rounded-xl font-semiboldbold transition-all duration-300 items-center justify-between gap-2 text-sm font-bold">
