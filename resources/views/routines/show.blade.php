...
<x-layout :title="$routine->name">
    <section class=" pt-10 pb-20 bg-white dark:bg-[#2A4043] rounded-t-3xl">
        <section class="mb-6 px-5">
            <div class="flex items-center justify-between">
                                        <div class="flex items-center">
                                            <h2 class="text-3xl font-medium text-[#306067] dark:text-[#CCE2E5]">{{ $routine->name }}</h2>
                                            @if($routine->routineTime)
                                                    @if($routine->routineTime?->name === 'Día')
                                                        <svg xmlns="http://www.w3.org/2000/svg" height="26px" viewBox="0 -960 960 960"
                                                            width="26px" fill="#37A0AF" class="ms-1 fill-[#37A0AF] dark:fill-[#E9E5E3]">
                                                            <path
                                                    ...
                                                    @elseif($routine->routineTime?->name === 'Noche')
                                                        <svg xmlns="http://www.w3.org/2000/svg" height="26px" viewBox="0 -960 960 960" width="26px" class="fill-[#37A0AF] dark:fill-[#E9E5E3] ms-1"><path d="M480.24-116.41q-153.63 0-258.73-104.98Q116.41-326.37 116.41-480q0-133.93 84.74-235.43t223.31-123.05q15.39-3.43 27.54 1.35 12.15 4.78 19.95 14.02 7.79 9.24 9.6 22.2 1.82 12.95-4.75 26.11-13.89 25.04-21.31 51.65-7.42 26.61-7.42 55.5 0 91.69 64.32 155.88 64.33 64.18 156.22 64.18 28.37 0 56.48-7.44 28.11-7.45 50.91-20.58 12.91-5.8 25.13-4.11 12.22 1.7 21.1 8.13 9.88 6.44 14.66 18.23 4.78 11.8 1.59 27.95Q820.17-291 717.63-203.71q-102.54 87.3-237.39 87.3Zm0-91q81.78 0 147.84-43.72 66.05-43.72 98.29-114.78-17.61 4.04-35.1 6.32-17.49 2.29-34.86 1.81-122.04-4.07-207.94-89.37-85.9-85.31-90.45-209.26-.24-17.37 1.93-34.98 2.16-17.61 6.44-34.98-70.82 32.48-114.78 98.65-43.96 66.18-43.96 147.72 0 112.93 79.83 192.76 79.83 79.83 192.76 79.83Zm-13.11-259.48Z"/></svg>
                                                    @endif
                                            @endif
                                        </div>
                                        <button onclick="event.stopPropagation(); document.getElementById('menu_rutina_{{ $routine->id }}').showModal()">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" class="fill-[#2A4043] dark:fill-[#CCE2E5]"><path d="M480.12-149q-34.55 0-59.13-24.55-24.58-24.56-24.58-59.04 0-34.58 24.56-59.2 24.55-24.62 59.03-24.62 34.67 0 59.13 24.59 24.46 24.6 24.46 59.13 0 34.54-24.46 59.11Q514.67-149 480.12-149Zm0-247.41q-34.55 0-59.13-24.56-24.58-24.55-24.58-59.03 0-34.67 24.56-59.13 24.55-24.46 59.03-24.46 34.67 0 59.13 24.46t24.46 59.01q0 34.55-24.46 59.13-24.46 24.58-59.01 24.58Zm0-247.18q-34.55 0-59.13-24.64-24.58-24.64-24.58-59.25t24.56-59.06Q445.52-811 480-811q34.67 0 59.13 24.46 24.46 24.45 24.46 59.06t-24.46 59.25q-24.46 24.64-59.01 24.64Z"/></svg>
                                        </button>
                                    </div>
            ...
            @forelse($steps as $step)
                ...
                <div>
                    <div class="flex justify-between items-center px-3 py-1 border-b-2 border-[#CCE2E5]">
                        ...
                        <button onclick="event.stopPropagation(); document.getElementById('desc_paso_{{ $step['number'] }}').showModal()">
                    ...
                    <div class="flex flex-col gap-3">
                        @foreach($step['products'] as $product)
                            ...
                            <!-- MODAL DESCRIPCIÓN PASOS -->
                            <dialog id="desc_paso_{{ $step['number'] }}" class="modal">
                            <div class="modal-box bg-white dark:bg-[#2A4043]">
                                <h3 class="text-lg font-bold text-[#306067] dark:text-[#CCE2E5]">Paso {{ $step['number'] }}: {{ $step['title'] }}</h3>
                                <p class="py-4 text-[#2A4043] dark:text-[#E9E5E3]">{{ $step['products']->first()->category?->description }}</p>
                            </div>
    ...
    <dialog id="menu_rutina_{{ $routine->id }}" class="modal modal-bottom">
        <div class="modal-box bg-white dark:bg-[#2A4043]">
            ...
            <a href="{{ route('routines.edit', $routine) }}" class="mt-3 btn w-full inline-flex border-0 bg-[#CCE2E5] px-6 py-3 rounded-xl font-semiboldbold transition-all duration-300 items-center justify-between gap-2 text-sm font-bold">
