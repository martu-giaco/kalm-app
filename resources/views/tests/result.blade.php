...
<body class="min-h-screen bg-center bg-cover" style="background-image: url('{{ asset('images/fondo.png') }}');">
    ...
    <div class="max-w-3xl px-7 py-8 mx-auto">
        <article class="pt-6">
            ...
            <div class="mb-7">
                ...
                <p class="text-center text-md text-[#37A0AF]">Según tus respuestas, tu tipo es <span class="font-bold font-3xl text-center text-[#306067] mb-4">{{ ucfirst($resultLabel) }}</span></p>
            ...
            <div class="mb-7">
                ...
                @if($routine->routineTime)
                    ...
                    @endif
                @endif --}}
                <p class="text-red-600 mb-5">acá va la rutina con sus detalles, como todavía no estoy segura como funca eso lo dejo a medio hacer D: perdón -mar g</p>

                @auth
                    <a href="{{ route('tests.createRoutine') }}" class="block px-4 py-3 bg-[#37A0AF] text-white rounded-lg w-full text-center">
                        Guardar Rutina
                    </a>
                @endauth
    ...
    {{-- Botones de acción --}}
            <div class="flex flex-wrap gap-3 mt-6 p-4 py-7 bg-white w-full">
                ...
                @auth
                    <form action="{{ route('tests.saveResult') }}" method="POST" class="inline w-full">
                        ...
                        <button type="submit" class="block px-4 py-3 bg-[#164d4f] text-white rounded-lg w-full">Guardar resultado en perfil</button>
                    </form>
                @else
