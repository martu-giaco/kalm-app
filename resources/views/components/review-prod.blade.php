        ...
        <div class="flex justify-between items-center gap-2 mb-2">
            ...
            <div class="flex items-center gap-2">
                ...
                <button onclick="event.stopPropagation(); document.getElementById('menu_review_{{ $review->id }}').showModal()">
                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#306067" class="fill-[#306067] dark:fill-[#CCE2E5]">
...
@if (auth()->check() && (auth()->id() === $review->user_id || auth()->user()->role === 'admin'))
    <dialog id="menu_review_{{ $review->id }}" class="modal modal-bottom">
        <div class="modal-box bg-white dark:bg-[#2A4043]">
            <a href="{{ route('reviews.edit', $review) }}" class="btn w-full inline-flex border-0 bg-[#CCE2E5] px-6 py-3 rounded-xl font-bold transition-all duration-300 items-center justify-between gap-2 text-sm">
            ...
            <form action="{{ route('reviews.destroy', [$review->product->id, $review->id]) }}" method="POST" onsubmit="return confirm('¿Seguro que querés eliminar esta reseña? Esta acción no se puede deshacer.')">
                @csrf
                @method('DELETE')
                <button type="submit" class="mt-3 btn w-full inline-flex border-0 bg-[#741919] px-6 py-3 rounded-xl font-bold transition-all duration-300 items-center justify-between gap-2 text-sm">
                    <p class="font-semibold text-white">Eliminar Reseña</p>
                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#CCE2E5">
