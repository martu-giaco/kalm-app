@php
    // --- Momento del día ---
    $timeName = $rutina->routineTime?->name; // 'Día' | 'Noche'

    // --- Tipo de rutina (Skincare / Haircare) ---
    $typeName = $rutina->type?->name; // ajustar si el accessor real es distinto
    ...
    // --- Recordatorio ---
    $reminderOn = (bool) ($rutina->is_reminder_enabled ?? false);
    $reminderTime = $rutina->reminder_time ? \Illuminate\Support\Carbon::parse($rutina->reminder_time)->format('H:i') : null;
    $reminderFrequency = $rutina->reminder_frequency ?? 'none';
    ...
    if ($reminderOn) {
        ...
        } elseif ($reminderFrequency === 'specific_days' && !empty($rutina->reminder_days)) {
            $days = is_array($rutina->reminder_days) ? $rutina->reminder_days : (json_decode($rutina->reminder_days, true) ?? []);
            $labels = collect($days)->map(fn($d) => $dayLabels[strtolower((string) $d)] ?? $d)->values();
            $frequencyLabel = $labels->implode(' · ');
        } elseif ($reminderFrequency === 'every_x_days' && $rutina->reminder_interval) {
            $frequencyLabel = 'Cada ' . $rutina->reminder_interval . ' días';
        } else {
...
<article
    onclick="window.location='{{ route('routines.show', $rutina) }}'"
    class="group relative flex flex-col w-full p-4 mb-3 overflow-hidden transition-all duration-300 bg-white dark:bg-[#243B3E] border border-[#E4EEEF] dark:border-[#3A5559] rounded-2xl shadow-sm cursor-pointer hover:shadow-lg hover:-translate-y-0.5"
>
    {{-- franja de acento según momento del día --}}
    <div class="absolute inset-y-0 left-0 w-1.5 {{ $timeName === 'Noche' ? 'bg-[#2A4043]' : 'bg-[#F5B942]' }}"></div>

    <div class="pl-2">
        {{-- Header: nombre + menú --}}
        <div class="flex items-start justify-between gap-2">
            <div class="flex items-center gap-2 min-w-0">
                <h2 class="text-lg font-semibold leading-tight text-[#306067] dark:text-[#CCE2E5] truncate">
                    {{ $rutina->name }}
                </h2>

                @if($timeName)
                    <span class="inline-flex items-center justify-center shrink-0 w-6 h-6 rounded-full
                        {{ $timeName === 'Noche' ? 'bg-[#2A4043]' : 'bg-[#FFF3D6]' }}"
                        title="{{ $timeName }}">
                        @if($timeName === 'Día')
                            {{-- sol --}}
                            <svg xmlns="http://www.w3.org/2000/svg" height="14px" viewBox="0 -960 960 960" width="14px" class="fill-[#C07A00]">
                        ...
                        @elseif($timeName === 'Noche')
                            {{-- luna --}}
                            <svg xmlns="http://www.w3.org/2000/svg" height="14px" viewBox="0 -960 960 960" width="14px" class="fill-[#CCE2E5]">
            ...
            <button
                onclick="event.stopPropagation(); document.getElementById('menu_rutina_{{ $rutina->id }}').showModal()"
                class="shrink-0 p-1 rounded-full hover:bg-[#F1F6F6] dark:hover:bg-[#2A4043] transition-colors"
            >
                <svg xmlns="http://www.w3.org/2000/svg" height="22px" viewBox="0 -960 960 960" width="22px" class="fill-[#306067] dark:fill-[#CCE2E5]">
        ...
        {{-- Chips: tipo (skincare/haircare) + necesidad --}}
        <div class="flex flex-wrap items-center gap-1.5 mt-2">
            @if($typeName)
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-[#E5F4F0] dark:bg-[#2E4A46] text-[#2E7D6B] dark:text-[#9FD8C6]">
                    @if($isHaircare)
                        {{-- ícono peine (haircare) --}}
            ...
            @if($rutina->routineNeed?->name)
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[#EEF3F4] dark:bg-[#2A4043] text-[#37A0AF] dark:text-[#37A0AF]">
                    {{ $rutina->routineNeed->name }}
                </span>
            @endif

            @if($reminderOn)
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-[#FFF3D6] dark:bg-[#4A3F20] text-[#8A6300] dark:text-[#F5D67D]">
                    {{-- ícono campana --}}
                    <svg xmlns="http://www.w3.org/2000/svg" height="12px" viewBox="0 -960 960 960" width="12px" class="fill-current">
            ...
            @if($frequencyLabel)
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-[#EAF7FB] dark:bg-[#213C40] text-[#1E7E96] dark:text-[#7FD3E8]">
                    {{-- ícono repetición --}}
                    <svg xmlns="http://www.w3.org/2000/svg" height="12px" viewBox="0 -960 960 960" width="12px" class="fill-current">
            ...
            <div class="flex items-center -space-x-2">
                @forelse($visible as $product)
                    <img
                        ...
                        alt="{{ $product->name }}"
                        class="object-contain w-12 h-12 bg-white border-2 border-white dark:border-[#243B3E] rounded-full shadow-sm"
                    >
                @empty
                    <p class="text-sm text-[#8FA6A9] dark:text-[#8FB1B4]">
                        No hay productos en esta rutina.
                    </p>
                @endforelse
                @if($remaining > 0)
                    <span class="flex items-center justify-center w-12 h-12 text-sm font-bold rounded-full border-2 border-white dark:border-[#243B3E] bg-[#E4EEEF] dark:bg-[#2A4043] text-[#2A4043] dark:text-[#E9E5E3]">
                        +{{ $remaining }}
                    </span>
                @endif
            </div>

            <svg xmlns="http://www.w3.org/2000/svg" height="22px" viewBox="0 -960 960 960" width="22px" class="fill-[#306067] dark:fill-[#CCE2E5] shrink-0 transition-transform group-hover:translate-x-0.5">
...
<dialog id="menu_rutina_{{ $rutina->id }}" class="modal modal-bottom">
    <div class="modal-box bg-white dark:bg-[#2A4043]">
        ...
        <a href="{{ route('routines.edit', $rutina) }}" class="mt-3 btn w-full inline-flex border-0 bg-[#CCE2E5] px-6 py-3 rounded-xl font-semibold transition-all duration-300 items-center justify-between gap-2 text-sm">
