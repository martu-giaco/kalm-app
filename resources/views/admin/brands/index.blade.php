...
<x-layout :title="'Marcas - Panel de Administración'">
    <div class="container my-5 bg-white rounded-t-3xl min-h-full pt-5">
        <div class="flex justify-between items-center mb-4 px-5">
            ...
            @auth
            <a href="{{ route('admin.brands.create') }}" class="btn font-bold text-white bg-[#306067] border-[#306067] btn-primary mb-3">Crear Marca</a>
            @endauth
        </div>
            ...
            <tbody>
                @foreach($brands as $brand)
                    <tr>
                        ...
                        <td>{{ $brand->name }}</td>
                        <td>
                            <a href="{{ route('admin.brands.view', $brand) }}" class="btn font-bold text-white bg-[#37A0AF] border-[#37A0AF] btn-info btn-sm">
