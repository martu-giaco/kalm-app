...
<x-layout>
    <div class="container my-5 px-5 bg-white rounded-t-3xl min-h-full pt-5">
        ...
        <h1 class="text-3xl font-bold text-[#306067] mb-4">Crear marca</h1>
        ...
        <form action="{{ route('admin.brands.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <div id="image-preview-container" class="my-3 hidden">
                    ...
                    <img id="image-preview" class="rounded-full h-80 w-80" />
                </div>
                <label for="logo"
                                ...
                                <p>Subir una foto</p>
                                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#2A4043">
                                    <path
                ...
                <input style="display: none;" type="file" id="logo" name="logo" class="w-full p-3 mb-3 bg-transparent rounded-xl border-2 border-[#CCE2E5] placeholder-[#CCE2E5] focus:outline-[#37A0AF] text-md text-[#2A4043] @error('logo') is-invalid @enderror" accept="image/*">
                @error('logo')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>

            <button type="submit" class="btn w-full px-5 py-3 rounded-xl text-white font-bold transition cursor-pointer disabled:opacity-80 disabled:cursor-not-allowed hover:bg-[#306067] bg-[#306067]">Crear marca</button>
        </form>
    </div>
