<x-layout>
    ...
    <section class="min-h-screen px-5 pt-7 pb-10 bg-white dark:bg-[#2A4043] rounded-t-3xl">
        <div>
            {{-- Cabecera resultados --}}
            <div class="flex items-start justify-between w-full gap-4">
                <div class="flex justify-between w-full">
                    ...
                    @endphp
                    @if (!empty($searchTerm))
                        <p class="text-md font-bold text-[#306067] dark:text-[#E9E5E3] mb-1">
                            {{ $total }}
                            resultado{{ $total > 1 ? 's' : '' }}
                    ...
                    <div class="p-0 modal-box bg-white dark:bg-[#2A4043]">
                        <div class="p-4 border-b-[1px] border-b-[#CCE2E5]">
                            <form method="dialog">
                                <button
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 fill-[#306067] dark:fill-[#CCE2E5]" viewBox="0 -960 960 960"
                                         aria-hidden="true">
                                        <path
                            ...
                            <h3 class="text-xl font-semibold text-center text-[#306067] dark:text-[#E9E5E3]">Filtros</h3>
                        </div>
                        ...
                        <form action="{{ route('blog.search') }}" method="GET">
                            ...
                            <section class="max-h-[60vh] overflow-y-auto">
                                <div class="mb-6">
                                    <h3 class="text-lg mb-5 p-4 border-y text-[#2A4043] dark:text-[#E9E5E3] font-semibold">
                                        Categoría
                                    </h3>
        ...
        <section>
            @foreach($blogsByTag as $tagName => $blogs)
                <h2 class="text-2xl font-bold text-[#306067] dark:text-[#CCE2E5] mt-5 mb-2">{{ $tagName }}</h2>
            ...
            @foreach($blogsByType as $typeName => $blogs)
                <h2 class="text-2xl font-bold text-[#306067] dark:text-[#CCE2E5] mt-5 mb-2">{{ $typeName }}</h2>
                <div class="flex gap-5 pb-5 overflow-x-auto scroll-smooth scrollbar-hide">
                            @foreach($blogs as $blog)
                                <x-blog-card :blog="$blog" />
                            @endforeach
